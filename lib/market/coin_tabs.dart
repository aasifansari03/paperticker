...
import 'dart:convert';
import '../components/flutter_candlesticks.dart';

import '../portfolio/transaction_sheet.dart';
import '../main.dart';
import '../services/functions/globalFunctions.dart';
...
class CoinDetailsState extends State<CoinDetails>
  ...
  _makeTabs() {
    if (widget.enableTransactions) {
      _tabAmt = 2;
      _tabBarChildren = [new Tab(text: "Stats"), new Tab(text: "Transactions")];
    } else {
      _tabAmt = 2;
      _tabBarChildren = [
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
        ...
        appBar: AppBar(
          ...
          title: new Text(
            widget.enableTransactions ? "Transactions" : "Exchanges",
          ),
    ...
    //         ? [aggregateStats(context), transactionPage(context)]
        ...
        children: <Widget>[
          ...
          new Flexible(
            child: historyOHLCV != null
                ? new Container(
                    padding:
                        const EdgeInsets.only(left: 2.0, right: 1.0, top: 10.0),
